...
describe('imei conformance', () => {
  for (const v of vectors) {
    it(`imei: ${v.input}`, () => {
      const allowSv = v.allowSv ?? false;
      const r = Imei.validate(v.input, { allowSv });
      ...
      if (v.format !== undefined && r.ok) expect(Imei.format(v.input, { allowSv })).toBe(v.format);
      if (v.parse) {
        const info = Imei.parse(v.input, { allowSv })!;
        ...
        expect(info.softwareVersion).toBe(v.parse.softwareVersion ?? null);
      }
    });
  }
});
